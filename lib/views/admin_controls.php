...
<!-- TODO make this a bootstrap navbar -->
	...
	<div class='btn-group'>
		...
		<a class='btn btn-inverse' id='logout' title='Logout' href='#logout'><i class='icon-off'></i> Logout</a>
		<a class='btn btn-inverse' id='delete-page'  title='Delete this page' href='#'> <i class='icon-trash'></i> Delete page</a>
		<?php if($_GET['preview']) { ?>
			<a href='.' class='btn btn-inverse'><i class='icon-edit'></i> Edit mode</a>
		<?php } else { ?>
			<a class='btn btn-inverse' id='delete-page' title='Delete this page' href='?preview=true' target="_blank"> <i class='icon-eye-open'></i> Preview</a>
		<?php } ?>
	...
	<div class='btn disabled hide' id='saving-status'></div>
		...
		<script type="text/template" class='toolbar-template'>
			<div class='btn-group wysihtml5-toolbar' id='<%= toolbar_id %>' style='display:none;'>
				...
				<a class='btn btn-inverse disabled'><%= field_name.replace("_"," ") %>: </a>
			...
			<a data-field-name='<%= field_name %>' data-field-type='<%= field_type %>' class='btn btn-success save-field-btn'><i class='icon-check'></i> Save</a>
			</div>
